...
export default function InventoryImportModal({ onClose, onImported }: Props) {
  ...
  const handleImport = async () => {
    ...
    const items = rows.map(row => ({
      name: row.name,
      preset: row.category?.toUpperCase() || "MODEL",
      shortName: row.short_name || null,
      shorthand: row.shorthand || null,
      manufacturer: row.manufacturer || null,
      standardDayRate: row.day_rate ? parseFloat(row.day_rate) : null,
      replacementCost: row.replacement_cost ? parseFloat(row.replacement_cost) : null,
      size: row.size || null,
      narrativeDescription: row.narrative_description || null,
      purchaseCost: row.purchase_cost ? parseFloat(row.purchase_cost) : null,
      countryOfManufacture: row.country_of_manufacture || null,
    ...
    try {
      ...
      if (result.success > 0) {
        // Re-fetch to get the created items — fetch all inventory and pass new ones
        const invRes = await fetch("/api/inventory");
        const allItems = await invRes.json();
        if (Array.isArray(allItems)) {
          onImported(allItems.slice(0, result.success));
        }
      }
      setStep("done");
    } catch {
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              <div>
                <p style={{ fontSize: 14, color: "#888", margin: "0 0 16px", lineHeight: 1.6 }}>
                  Upload a CSV or Excel file to bulk-import inventory items. Download the template to see the required format.
                </p>
                <button
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: "#111", borderBottom: "1px solid #242424" }}>
                        {["Name", "Category", "Manufacturer", "Day Rate", "Replacement Cost"].map(h => (
                    ...
                    <tbody>
                      {previewRows.map((row, i) => (
                        <tr key={i} style={{ borderBottom: "1px solid #1e1e1e" }}>
                          ...
                          <td style={{ padding: "10px 16px", fontSize: 13, color: "#666" }}>{row.category || "—"}</td>
                          <td style={{ padding: "10px 16px", fontSize: 13, color: "#666" }}>{row.manufacturer || "—"}</td>
                          <td style={{ padding: "10px 16px", fontSize: 13, color: row.day_rate ? "#e8a045" : "#333", fontFamily: "monospace" }}>
                            {row.day_rate ? `$${row.day_rate}` : "—"}
                          </td>
                          <td style={{ padding: "10px 16px", fontSize: 13, color: "#666", fontFamily: "monospace" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16, padding: "32px 0", textAlign: "center" }}>
              ...
              <div>
                <p style={{ fontSize: 18, fontWeight: 700, color: "#f0f0f0", margin: "0 0 8px" }}>
                  {importResult.success} {importResult.success === 1 ? "item" : "items"} imported
                </p>
          ...
          {step === "preview" && (
            <>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                >
                  {importing ? "Importing..." : `Import ${rows.length} ${rows.length === 1 ? "Item" : "Items"}`}
                </button>
              </div>
            </>
          )}
